...
function writeRecord(sessionId, record) {
  try {
    ...
    // 每个 response 独占一个文件，不用 append，避免多个调用把内容交错写入；代码未设置大小上限。
...
// 在覆盖前保存宿主原始 fetch；所有请求最终都必须且只调用这个函数一次。
const origFetch = globalThis.fetch;
// 极旧/非 Fetch 运行时没有全局 fetch 时保持静默，本 preload 不安装任何替代实现。
if (typeof origFetch === 'function') {
  /**
   * fetch 的旁路观察包装器。
   *
   * 非 Messages API 或缺少 session header 时立即委托原 fetch。目标请求会先读取可安全观察的 system，
   * 再等待原响应，并用 TransformStream 在下游实际消费响应 body 时解析 SSE。`async` 表示它始终返回
   * Promise；原网络拒绝会原样向调用方传播，旁路失败尽量回退原响应。
   *
   * @param {string|object} input Fetch URL 或 Request-like 对象。
   * @param {object|undefined} init Fetch 初始化参数。
   * @returns {Promise<Response>} 原始响应，或正文经过等价旁路 TransformStream 的新 Response。
   * @throws 原始 fetch 的网络/协议异常原样向 Claude Code 传播。
   */
  globalThis.fetch = async function patchedFetch(input, init) {
    let url;
    try {
      // 兼容 fetch(url, init)、fetch(Request) 和其他可转字符串的输入形式。
      url = typeof input === 'string' ? input
          : (input && typeof input === 'object' && typeof input.url === 'string') ? input.url
          : String(input);
    } catch (_) {
      // 连 String(input) 都失败时视为非目标请求，仍交给原 fetch 决定其真实异常行为。
      url = '';
    }

    // 快速旁路：普通网络请求不读取 header/body，也不包装响应流。
    if (!url || !LLM_URL_RE.test(url)) {
      // call(this, ...) 保留宿主调用 fetch 时的 this 绑定。
    ...
    try {
      // init 中显式 headers 优先；否则从 Request-like input 读取。
      const headers = dumpHeaders(
        init?.headers ?? (input && typeof input === 'object' ? input.headers : null),
      );
      // 这是 Claude Code 自动加入的 HTTP header，不是进程环境变量。
      sessionId = headers['x-claude-code-session-id'] || null;
      if (sessionId) {
        // 与 header 相同，init.body 优先于 Request-like input.body；读取函数不会消费流式 body。
        const body = init?.body
          ?? (input && typeof input === 'object' ? input.body : null);
        systemInstructions = safeParseRequestSystem(body);
      }
      // header/body 观察异常按缺失补充数据处理，真实请求仍在下方照常发出。
    } catch (_) {}

    if (!sessionId) {
      // 没有隔离键就不能生成可与 Stop transcript 安全关联的路径，因此完全放弃截获。
      return origFetch.call(this, input, init);
    }

    // performance.now() 是单调时钟，适合测持续时间；起点在 await 原 fetch 之前，包含首包等待。
    const startMs = performance.now();
    let response;
    try {
      // await 只暂停当前 patchedFetch Promise，不阻塞 Bun 事件循环中的其他异步工作。
    ...
    // responseId 来自 message_start；没有它就无法命名文件或与 transcript 关联。
    let responseId = null;
    // ttftNs 在第一个有效内容增量出现时填写；整流没有增量则保持 null。
    let ttftNs = null;
    // 防止同一响应因多个 chunk/flush 重复尝试写同一中间记录。
    let recordWritten = false;
    // 写出所需字段后停止旁路解码，但响应字节仍会继续原样 enqueue 给 Claude Code。
    let stopParsing = false;
    // TextDecoder 以 stream 模式跨 chunk 保留不完整 UTF-8 字符，避免分块边界产生乱码。
    const decoder = new TextDecoder();
    // 网络 chunk 不等于 SSE event；pending 暂存尚未遇到空行边界的文本。
    let pending = '';

    /**
     * 最多尝试写一次当前响应的中间记录。
     *
     * `writeRecord()` 自身吞掉文件异常，因此调用后无论是否真正落盘都会把 recordWritten 设为 true；
     * 这是 fail-open 设计，避免网络流中反复同步重试磁盘故障。message_start 先到时通常等 TTFT；
     * 整流结束仍无 delta 时由 flush 带 `ttft_ns:null` 调用。
     *
     * @returns {void}
     */
    const tryEmit = () => {
      // 必须同时有稳定 response ID，且本次响应还没有尝试保存。
    ...
    /**
     * 消费一个完整 SSE 事件块，只提取 response ID 和首内容时间。
     * @param {string} block 已按空行边界切出的 SSE 文本。
     * @returns {void}
     */
    const processBlock = (block) => {
      const parsed = parseSseBlock(block);
      if (!parsed) return;
      // 只接受第一个合法 message_start ID，后续重复事件不会改写文件关联键。
    ...
    let transform;
    try {
      // TransformStream 位于原 response.body 与 Claude Code 消费者之间，维持标准流式背压。
      transform = new TransformStream({
        /** 每收到一个二进制 chunk，先透传，再尽力把完整 SSE 事件交给 processBlock。 */
        transform(chunk, controller) {
          // TransformStream 按下游拉取节奏调用 transform；先 enqueue 同一 chunk，保留原始字节和背压链。
          controller.enqueue(chunk); // 先透传数据；解析仅为尽力而为的旁路操作。
          // 已取得 response ID 和 TTFT 后不再增长 pending，降低长回答的内存与 CPU 开销。
          if (stopParsing) return;
          try {
            // `{stream:true}` 告诉 decoder 后续还有字节，不要把 chunk 尾部半个字符替换成乱码。
            pending += decoder.decode(chunk, { stream: true });
            let idx;
            // 一个 chunk 可能包含多个 SSE 事件；循环逐个消费，末尾半个事件继续留在 pending。
            while ((idx = pending.indexOf(SSE_DELIMITER)) !== -1) {
              const block = pending.slice(0, idx);
              pending = pending.slice(idx + SSE_DELIMITER.length);
              processBlock(block);
            }
            // 两个关键字段齐备就立即落盘，不必等可能持续很久的响应流结束。
            if (responseId && ttftNs !== null) {
              tryEmit();
              stopParsing = true;
              // 已不再解析后续流，释放未消费的事件文本引用。
              pending = '';
            }
            // 解码/SSE/旁路落盘异常不影响已经 enqueue 的真实响应数据。
          } catch (_) {}
        },
        /** 下游读完整个响应流时由 Streams API 调用，用于保存只有 message_start 的部分记录。 */
        flush() {
    ...
    try {
      // 构造尽量等价的响应：保留 status/statusText/headers/body，但不会复制原对象的 url、
      // redirected、type 等其他只读元数据；待确认 Claude Code 是否依赖这些属性。
      ...
      // 新 Response 构造失败时尽力返回原对象，让宿主看到的错误行为不由采集逻辑主动改变。
      return response;
    }
  };
}
